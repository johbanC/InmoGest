...
class FiadorController extends Controller
{
    ...
    {
        $tipoFiador = \App\Models\TipoCliente::where('acronimo', 'FIA')->first();
        $clientes = [];
        if ($tipoFiador) {
            $clientes = Cliente::with(['tipoDocumento', 'tipoEstatus']) // Agrega la relación tipoEstatus
                ->where('tipo_cliente_id', $tipoFiador->id)
        ...
        return view('fiadores.index', compact('clientes', 'tipoFiador', 'tipodocumentos', 'tipoestatus', 'tipoclientes'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $CrearFiador = Cliente::create([
                ...
                'tipo_cliente_id' => 3, // Por defecto 1 si no existe
                'tipo_estatus_id' => 1, // Estado activo
                'user_id' => auth()->id(),
            ]);

            if ($CrearFiador) {
                return to_route('fiadores.index')->with('status', 1);
            } else {
                return to_route('fiadores.index')->with(['status' => 2, 'error' => 'No se pudo crear el fiador.']);
            }
        } catch (\Exception $e) {
            // Puedes mostrar el error en la vista con @if(session('error')) ...
            return to_route('fiadores.index')->with(['status' => 2, 'error' => $e->getMessage()]);
        }
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            return to_route('fiadores.index')->with('status', 3);
        } catch (\Exception $e) {
            return to_route('fiadores.index')->with(['status' => 4, 'error' => $e->getMessage()]);
        }
    }
